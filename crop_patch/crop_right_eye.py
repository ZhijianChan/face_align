...
def crop(img, pts, crop_size = 160, sub_dist = 50):
    """
    crop face patch with right eye located at the image center
    ...
    er_nose_x = pts_[1,0] - pts_[2,0]
    er_nose_y = pts_[2,1] - pts_[1,1]
    max_sub = max(er_nose_x, er_nose_y)
    scale_ = sub_dist / max_sub
    er_x = int(pts_[1,0] * scale_)
    er_y = int(pts_[1,1] * scale_)
    if scale_ <= 0:
        return None
    ...
    (h_,w_,_) = img.shape
    # axis-X
    beg_x_dst = max(crop_size/2 - er_x, 0)
    beg_x_src = max(er_x - crop_size/2, 0)
    end_x_dst = min(crop_size, crop_size/2 + w_ - er_x)
    end_x_src = min(w_, er_x + crop_size/2)
    # axis-Y
    beg_y_dst = max(crop_size/2 - er_y, 0)
    beg_y_src = max(er_y - crop_size/2, 0)
    end_y_dst = min(crop_size, crop_size/2 + h_ - er_y)
    end_y_src = min(h_, er_y + crop_size/2)
    # copy
